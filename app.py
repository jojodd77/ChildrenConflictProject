...
def call_agent(system_prompt, user_message, agent_name="Agent", temperature=0.3):
    try:
        ...
        # 【核心抢救】：添加 timeout=7.0！
        # 强制大模型在7秒内必须回答，否则直接报错，绝不给 Vercel 触发10秒崩溃的机会！
        client = OpenAI(
            api_key=api_key,
...
def save_session(session):
    ...
    req = urllib.request.Request(url, data=payload, headers={"Authorization": f"Bearer {UPSTASH_TOKEN}"}, method='POST')
    try:
        with urllib.request.urlopen(req, timeout=3.0) as response:
            pass 
    except Exception as e:
...
def join():
    ...
    if session["scenario"] is None:
        ...
        if scenario_res:
            ...
            # Vercel 超时救命稻草：如果超过7秒大模型没反应，立刻丢出这个备用剧本，保住服务器不崩！
            session["scenario"] = {
                "title": "自然课上的植物标本",
